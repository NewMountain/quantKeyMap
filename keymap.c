...
[BASE] = KEYMAP(  // layer 0 : default
        ...
        KC_LCTL,   KC_LGUI,      KC_TRNS,  KC_LALT,MO(HAKR),
                                                    KC_TRNS,  KC_TRNS,
                                                              KC_HOME,
...
[HAKR] = KEYMAP(
       ...
       KC_TRNS, M(JS_FUNC),   KC_DLR, KC_LBRC, KC_RBRC,  KC_GRV, KC_TRNS,
       KC_TRNS,    KC_TRNS,  KC_TRNS, KC_TRNS, KC_TRNS,
                                           KC_TRNS, KC_TRNS,
                                                    KC_TRNS,
                                  KC_TRNS, KC_TRNS, KC_TRNS,
   ...
   M(EQUAL_TO),  KC_AMPR, KC_PPLS,    KC_LABK, KC_PAST,M(APPLICATIVE),      M(BIND),
                 KC_COLN, KC_UNDS,    KC_RABK,  M(FMAP),      KC_PEQL,M(DOC_STRING),
...
[EXZL] = KEYMAP(
       ...
       KC_LSFT, KC_TRNS,    KC_1,    KC_2,    KC_3, KC_TRNS, KC_TRNS,
       KC_LCTL, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
                                           KC_TRNS, KC_TRNS,
                                                    KC_TRNS,
                                  KC_TRNS, KC_TRNS, KC_TRNS,
    // right hand
       ...
       KC_TRNS,  KC_TRNS, KC_PPLS,   KC_UP, KC_PAST, KC_TRNS, KC_TRNS,
                 KC_TRNS, KC_LEFT, KC_DOWN, KC_RGHT, KC_PEQL, KC_TRNS,
  ...
  // MACRODOWN only works in this function
    ...
    case ASSIGN:
      if(record->event.pressed){
        return MACRO( T(LABK), T(PMNS), T(SPACE),END);
      }
      break;
    case JS_FUNC:
        if(record->event.pressed){
          return MACRO( T(PEQL), T(RABK), T(SPACE),END);
        }
        break;
    case B_PIPE:
        if(record->event.pressed){
          return MACRO( T(LABK), T(PIPE), T(SPACE),END);
        }
        break;
    case F_PIPE:
        if(record->event.pressed){
          return MACRO( T(PIPE), T(RABK), T(SPACE),END);
        }
        break;
    case JS_COMMENT:
        if(record->event.pressed){
          return MACRO( T(PSLS), T(PSLS), T(SPACE),END);
        }
        break;
    case EQUAL_TO:
        if(record->event.pressed){
          return MACRO( T(PEQL), T(PEQL), T(SPACE),END);
        }
        break;
    case NOT_EQUAL:
        if(record->event.pressed){
          return MACRO( T(EXLM), T(PEQL), T(SPACE),END);
        }
        break;
    case APPLY_TO:
        if(record->event.pressed){
          return MACRO( T(PMNS), T(RABK), T(SPACE),END);
        }
        break;
    case FMAP:
        if(record->event.pressed){
          return MACRO( T(LABK), T(DLR), T(RABK), T(SPACE),END);
        }
        break;
    case APPLICATIVE:
        if(record->event.pressed){
          return MACRO( T(LABK), T(ASTR), T(RABK), T(SPACE),END);
        }
        break;
    case HAS_TYPE:
        if(record->event.pressed){
          return MACRO( T(COLN), T(COLN), T(SPACE),END);
        }
        break;
    case BIND:
        if(record->event.pressed){
          return MACRO( T(RABK), T(RABK), T(PEQL), T(SPACE),END);
        }
        break;
    case DOC_STRING:
        if(record->event.pressed){
          return MACRO( T(DQUO), T(DQUO), T(DQUO), T(SPACE),END);
        }
        break;
    case TRIPLE_ZERO:
        if(record->event.pressed){
          RETURN MACRO( T(0), T(0), T(0), T(SPACE), END)
        }
      }
    return MACRO_NONE;
};
